...
export const getUserPublications = (name,currentPage) => async (dispatch) => {
    ...
    const {data}=await axios.post(`/publication/user-publications?page=${currentPage}`,{name},config);
...
export const getPublications = (keyword,currentPage,category,value,setValue,ppp,fYear,tYear,fMonth,currentYear) => async (dispatch) => {
  ...
  tYear=tYear==""?currentYear:tYear
    ...
    try {
        ...
        dispatch({type:"PUBLICATION_REQUEST"})
    const {data}=await axios.get(`/publication/publications?keyword=${keyword}&page=${currentPage}&typeOfPublication=${category=="All"?"":category}&citations[gte]=${value[0]}&citations[lte]=${value[1]}&ppp=${ppp}&fYear=${fYear}&tYear=${tYear}&fMonth=${fMonth}`,config);
    dispatch({type:"PUBLICATION_SUCCESS",payload:data})
...
export const getPublicationsHome=()=> async(dispatch)=> {
  ...
  dispatch({type:"PUBLICATION_REQUEST"})
const {data}=await axios.get(`/publication/publications-for-home`,config);
dispatch({type:"PUBLICATION_SUCCESS",payload:data})
...
export const getPublicationDetails = (id) => async (dispatch) => {
    ...
    dispatch({type:"PUBLICATION_DETAILS_SUCCESS",payload:data.publicationDetails})

    } catch (error) {
        dispatch({type:"PUBLICATION_DETAILS_FAIL",payload:error.response.data.message})
    }
  }
export const uploadPublications = (file) => async (dispatch) => {
    ...
    dispatch({type:"PUBLICATION_UPLOAD_SUCCESS",payload:data.publicationDetails})

    } catch (error) {
        dispatch({type:"PUBLICATION_UPLOAD_FAIL", payload:error.response.data.message})
    }
  }